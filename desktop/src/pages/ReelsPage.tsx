...
export default function ReelsPage({
        ...
        if (response.ok) {
          ...
          if (result.data?.posts) {
            ...
            if (videoPosts.length > 0) {
              setActiveVideoId((prev) => prev ?? videoPosts[0].id);
            }
  ...
  // Set up Intersection Observer to update active video when in view
  useEffect(() => {
  ...
  const scrollToPrevious = () => {
    ...
    const currentIndex = videos.findIndex((v) => v.id === activeVideoId);
    const prevIndex =
      currentIndex > 0 ? currentIndex - 1 : Math.max(videos.length - 1, 0);
                        ...
                        <Avatar
                          ...
                          onClick={() => {
                            if (video.user?.username) {
                              onNavigate(`profile/${video.user.username}`);
                            }
                          }}
                        >
                          <AvatarImage
